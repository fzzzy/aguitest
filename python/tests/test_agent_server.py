...
def test_process_attachments():
    ...
    assert any(
        isinstance(c, TextInputContent) and "hello.txt" in c.text
        for c in mock_msg.content
    )
    assert any(
        isinstance(c, BinaryInputContent) and c.filename == "image.png"
        for c in mock_msg.content
    )
...
async def test_ping_all_sessions(caplog):
    ...
    try:
        ...
        with (
            caplog.at_level(logging.ERROR, logger="agent_server"),
...
def test_tool_schema_to_a2ui_with_properties():
    schema = {
        "properties": {
            "name": {"title": "Name", "description": "The user name", "type": "string"},
...
async def test_event_stream_closes_cleanly_on_disconnect():
    ...
    async def event_stream():
        try:
            ...
            state["ag_ui_events"] = adapter.encode_stream(adapter.run_stream(deps=deps))
    ...
    assert loop_errors == [], "disconnect left background errors: " + "; ".join(
        repr(c.get("exception")) for c in loop_errors
    )
